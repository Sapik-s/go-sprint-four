...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	speedInM := math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)
	heightInM := height / cmInM
	return (walkingCaloriesWeightMultiplier*weight + (speedInM/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
